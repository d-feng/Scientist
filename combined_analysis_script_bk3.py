...
def convert_ids_to_symbols(gene_list):
    ...
    for gene in gene_list:
        if str(gene).startswith('ENSG'):
            # Human Ensembl ID; use full ID without trimming
            ensembl_id_base = str(gene)
            ...
            for i, term in enumerate(results_df.index):
                ...
                try:
                    ...
                    gseaplot.gseaplot(rank_metric=enr.ranking,
                                      ...
                                      ofname=plot_file,
                                      **enr.results[term])
                    print(f"Generated GSEA plot for term '{term}' to {plot_file}")
    ...
    for col in condition_columns:
        if col in adata.obs.columns:
            ...
            if deg_results:
                ...
                print(json.dumps(deg_results, indent=2))
